...
int go(Board board,float param[param_size]){
    ...
    if(board.point[0]+board.point[1]>40)std::cout<<"depth: "<<63-board.point[0]-board.point[1]<<std::endl;
    ...
    for(int i=0;i<moves.size();i++){
        ...
        //終盤20手で完全読み
        if(board.point[0]+board.point[1]>40)eval_ref=alphabeta(board_ref,param,63-board.point[0]-board.point[1],-inf,inf);
